...
세션 계층
=======
* 세션 : 앙측 터미널간 일회용 논리적 연결, 세션의 연결 / 관리 / 해제, 다중화를 통한 효율성 증대, 동기화
* 대화 관리자(Dialog Controller) : 전달할 데이터를 확인하거나, 사용자가 안전한 대화를 위해 서비스를 제공해준다
* 전송 모드를 결정 : 전이중, 반이중, 비동기, 병렬/직렬 연결
* 프로토콜
* * RPC(Remote Procedure Call) : Window에서 사용되는 원격 프로지서 호출 프로토콜

표현 계층
=======
* 애플리케이션이 정보를 잘 통신할 수 있도록 필터하고 변환하는 계층이다. 응용계층과 세션계층 사이에 위치하여, 각 계층이 상호작용을 쉽게 하도록 전달하는 일을 담당한다.
* 암호화 및 압축을 수행한다.
* 프로토콜
* * SSL(Secure Socket Layer) : 네트워크 레이어의 암호화 방식. HTTP, FTP 등에서 이용. 인증 / 암호화 / 무결성을 보장하는 프로토콜


응용 계층
=======
* 사용자에세 서비스를 제공하고, 사용자의 명령을 수행한다. WWW, SNMP, SMTP, POP, FTP, TELNET 등이 이애 속하는 계층이다.
* 프로토콜
* * HTTP(Hypertext Transfer Protocol) : WWW 상에서 HTML 문서 교환을 위한 프로토콜. 주로 80포트를 이용한다.
* * SMTP(Simple Mail Transfer Protocol) : 인터넷에서 메일을 주고받기 위한 프로토콜. 25번 포트 사용.
* * FTP(File Transfer Protocol) : 컴퓨터간 파일 전송을 위한 프로토콜. 데이터 전달은 20번 포트, 제어정보전달은 21번 포트를 사용.
